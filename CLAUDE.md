...
1. **Deterministic normalization**: Same raw value MUST always produce same canonical value. This is enforced by the two-tier lookup — never bypass `MappingsManager`.
...
6. **Mappings are append-only during processing**: Never delete or modify mappings during `extract_new`. Mappings can only be edited via `review_mappings` or `review_rejected` tasks.
