...
```
Documents with 0-1 QR codes have `"sub_documents": null`. `SubDocumentMetadata` model used for construction, `.model_dump()` for storage. Backfill: `python main.py backfill_sub_documents <processed_path>` (QR-only, no NIF enrichment). Use `sync --pattern "*shared-toll*"` afterward for full NIF-enriched sub-documents.
