...
```
Documents with 0-1 QR codes have `"sub_documents": null`. `SubDocumentMetadata` model used for construction, `.model_dump()` for storage. Backfill: `python main.py backfill sub-documents <processed_path>` (QR-only, no NIF enrichment). Use `sync --pattern "*via-verde*"` afterward for full NIF-enriched sub-documents.
...
| `qr-inventory` | Scan PDFs for QR codes, create inventory | `--export_path`, `--no_resume` |
...
| `validate-extraction` | Validate extraction quality, flag issues | `processed_path?`, `--pattern` |
| `copy` | Copy files matching pattern | `processed_path?`, `--pattern` (req), `--dest` (req) |
| `split-bundles` | Split bundled documents | `processed_path?`, `--dry_run` |
| `rename` | Rename files based on metadata | `processed_path?` |
| `validate` | Validate metadata consistency | `processed_path?` |
| `fix-unicode` | Fix escaped Unicode in metadata JSON | `processed_path?` |
| `audit` | Audit processed files | `processed_path?` |
