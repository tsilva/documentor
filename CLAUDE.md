...
2. **Phase 1 - Raw Extraction** (`classify_pdf_document`): Renders first 2 pages as JPEG, sends to LLM with vision. For `issuing_party`, extracts EXACTLY as appears. For `document_type`, extracts only the core type label (strips dates, billing periods, reference numbers)
...
Fields: `issue_date`, `document_type`, `issuing_party`, `service_name`, `total_amount`, `total_amount_currency`, `confidence`, `reasoning`, `content_hash`, `file_hash`, `create_date`, `update_date`, `document_type_raw`, `issuing_party_raw`, `document_type_key`, `issuing_party_key`, `page_count`, `issuer_tax_number`, `locale`, `qrcode`
...
Each profile is a self-contained folder under `profiles/`. Current setup: `profiles/default/profile.yaml`
